...
class PaperService:
    ...
    def upload_paper(self,title,author_id,file):
        logger.info(f"begin123")
        # 调用 DAO 层上传方法
        try:
            paper, error = self.paper_dao.upload_paper(title, author_id, file)
            if error:
                logger.error(f"上传失败: {error}")
                raise ValueError(error)  # 使用更具体的异常类型
            logger.info("begin456")
            #上传成功后记录操作
            ...
            logger.info(f"上传成功 - 文件: {paper.title}")
            return paper
        except Exception as e:
            logger.error(f"服务层异常: {str(e)}")
            raise  # 重新抛出异常，让控制器处理
