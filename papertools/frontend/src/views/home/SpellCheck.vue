...
const operatorId = ref<number>(userInfo.value.id || 1); 
...
const exportResult = async () => {
  ...
  // 记录操作到数据库
  await logOperation('导出纠正结果');
};

// 记录操作到数据库
const logOperation = async (operationType: string) => {
  try {
    const response = await axios.post('http://localhost:5000/api/operations//log_operation', {
      user_id: operatorId.value,
      paper_id: null, // 这里假设没有关联的论文 ID
      operation_type: operationType
    });

    console.log('操作记录成功:', response.data);
  } catch (error) {
